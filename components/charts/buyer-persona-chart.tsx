...
interface BuyerPersonaChartProps {
  data: Company[]
  className?: string
...
export function BuyerPersonaChart({ data, className, shortlistedIds, onCellClick }: BuyerPersonaChartProps) {
  ...
  const { cells, personas, yGroups, cellLookup } = useMemo(() => {
    const cellMap = new Map<string, CellData>()

    for (const company of data) {
      const persona = company.buyerPersona || "Unknown"
      const yValues = getYValues(company, yAxis)

      for (const yVal of yValues) {
        const key = `${persona}|||${yVal}`
    ...
    for (const c of allCells) personaCounts.set(c.persona, (personaCounts.get(c.persona) || 0) + c.count)
    const sortedPersonas = Array.from(personaCounts.entries()).sort((a, b) => b[1] - a[1]).map(([name]) => name)
    ...
    for (const cell of allCells) lookup.set(`${cell.persona}|||${cell.yGroup}`, cell)

    return { cells: allCells, personas: sortedPersonas, yGroups: sortedYGroups, cellLookup: lookup }
  }, [data, yAxis])
  ...
  useEffect(() => {
    ...
    const margin = { top: 10, right: 30, bottom: 120, left: 200 }
    const cellSize = Math.max(28, Math.min(50, (width - margin.left - margin.right) / personas.length))
    const innerWidth = personas.length * cellSize
    ...
    const xScale = d3.scaleBand<string>().domain(personas).range([0, innerWidth]).padding(0.06)
    ...
    for (const persona of personas) {
      for (const yGroup of yGroups) {
        const cell = cellLookup.get(`${persona}|||${yGroup}`)
        const x = xScale(persona) ?? 0
        ...
        g.append("rect").attr("x", x).attr("y", y).attr("width", w).attr("height", h)
          ...
          .on("mouseover", (event) => {
            ...
            tooltipRef.current.innerHTML = [
              `<strong>${cell.persona}</strong> × <strong>${cell.yGroup}</strong>`,
              `Startups: ${cell.count}`,
    ...
    g.append("g").attr("transform", `translate(0,${innerHeight})`)
      ...
      .attr("transform", "rotate(-45)").attr("fill", axisColor).style("font-size", "10px")
    ...
    const grad = defs.append("linearGradient").attr("id", "buyer-persona-legend-grad")
    ;[0, 0.25, 0.5, 0.75, 1].forEach(s => {
      grad.append("stop").attr("offset", `${s * 100}%`).attr("stop-color", d3.interpolatePurples(s))
    })
    legendG.append("rect").attr("width", legendWidth).attr("height", 8)
      .attr("fill", "url(#buyer-persona-legend-grad)").attr("rx", 2)
    ...
    <Card className={cn("flex flex-col", className)}>
      <div className="flex flex-wrap gap-3 items-center p-3 border-b border-border">
        ...
        <div className="text-xs text-muted-foreground">
          {personas.length} buyer personas × {yGroups.length} groups
        </div>
      </div>
